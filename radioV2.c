...
int32_t left_stick_val = 0;
int32_t right_stick_val = 0;
...
void WTimer2BIntHandler(void){
    ...
    right_stick_val = ((micro_sec[1]*3175)/10000) - 384;
    if (right_stick_val > 200){
        right_stick_val = 200;
    }
    if (right_stick_val < 0){
        right_stick_val = 0;
    }
...
void WTimer3AIntHandler(void){
    ...
    left_stick_val = ((micro_sec[2]*3175)/10000) - 384;
    if (left_stick_val > 200){
        left_stick_val = 200;
    }
    if (left_stick_val < 0){
        left_stick_val = 0;
...
void handle_alt_func(void){
    ...
    // check right stick left/right
    if (micro_sec[0] < alt_func_lower_lim){
        alt_func_count[2]++;
    } else if (micro_sec[0] > alt_func_upper_lim){
        alt_func_count[3]++;
    }
